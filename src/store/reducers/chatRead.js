...
export default function chatRead(
  ...
  switch (action.type) {
    ...
    case 'REC_CHAT_TYPING': {
      const { channel: cid, name, user, isTyping } = action;
      const now = Date.now();
      const chanTyping = { ...(state.typing[cid] || {}) };
      if (user) {
        if (isTyping) {
          chanTyping[user] = { name, ts: now };
        } else if (chanTyping[user]) {
          delete chanTyping[user];
        }
      }
      return {
        ...state,
