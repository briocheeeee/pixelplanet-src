...
import { TILE_SIZE, EVENT_USER_NAME, BOT_USER_NAME } from './constants.js';
...
class RpgEvent {
  ...
  async initialize() {
    ...
    if (socketEvents.important) {
       ...
       */
      setState({ void: { eventTimestamp, active: false } });
  ...
  async runEventLoop() {
    ...
    if (eventMinutes > STEPS[0]) {
      ...
      if (eventState !== 11) {
        ...
        const [i, j] = this.eventCenterC;
        const { size: canvasSize } = canvases[CANVAS_ID];
        const cx = i * TILE_SIZE - canvasSize / 2;
        const cy = j * TILE_SIZE - canvasSize / 2;
        const link = `#d,${cx},${cy},30`;
        setState({ void: { active: true, startedAt: Date.now(), center: { x: cx, y: cy }, link } });
        if (chatProvider.enChannelId && chatProvider.botUserId) {
          chatProvider.chatMessageBuffer.broadcastChatMessage(
            BOT_USER_NAME,
            `void at ${link}`,
            chatProvider.enChannelId,
            chatProvider.botUserId,
            '/avatars/6.webp',
          );
        }
        this.eventState = 11;
        ...
        if (percent === 100) {
          ...
          const [x, y, w, h] = this.eventArea;
          await protectCanvasArea(CANVAS_ID, x, y, w, h, true);
          setState({ void: { active: false, endedAt: Date.now() } });
        } else {
          const now = Date.now();
