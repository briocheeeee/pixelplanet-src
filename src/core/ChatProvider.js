...
export class ChatProvider {
  ...
  async initialize() {
    ...
    this.botUserId = await getDummyUser(BOT_USER_NAME);
  }
  ...
  async sendMessage(user, ip, message, channelId, lang, ttag) {
    ...
    let displayAvatar = (user && user.data && user.data.avatar)
      ? user.data.avatar
      : (user && user.avatar) ? user.avatar : null;
    if (typeof displayAvatar === 'string') {
      if (displayAvatar.startsWith('/public/avatars/')) {
        displayAvatar = displayAvatar.replace('/public/avatars/', '/avatars/');
      } else if (displayAvatar.startsWith('avatars/')) {
    ...
    this.handleBotCommands(message, channelId);
    return null;
  }
  ...
  broadcastBotMessage(message, channelId) {
    const cid = channelId || this.enChannelId;
    if (cid && this.botUserId) {
      this.chatMessageBuffer.broadcastChatMessage(
        BOT_USER_NAME,
        message,
        cid,
        this.botUserId,
  ...
  handleBotCommands(message, channelId) {
    const q = message.trim().toLowerCase();
    if (q === 'when void') {
      const st = getState();
      const v = (st && st.void) ? st.void : {};
      const now = Date.now();
      if (v && v.active) {
        const ts = (v.startedAt) ? Math.floor(v.startedAt / 1000) : Math.floor(now / 1000);
        const link = (v && v.link)
          ? v.link
          : (v && v.center && typeof v.center.x === 'number')
            ? `#d,${v.center.x},${v.center.y},30`
            : '';
        const msg = link ? `void started <t:${ts}:R> at ${link}` : `void started <t:${ts}:R>`;
        this.broadcastBotMessage(msg, channelId);
        return;
      }
      if (v && v.eventTimestamp && now < v.eventTimestamp) {
        const ts = Math.floor(v.eventTimestamp / 1000);
        this.broadcastBotMessage(`void starts <t:${ts}:R>`, channelId);
        return;
      }
      if (v && v.endedAt && now > v.endedAt) {
        const ts = Math.floor(v.endedAt / 1000);
        this.broadcastBotMessage(`void already passed <t:${ts}:R>`, channelId);
        return;
      }
      this.broadcastBotMessage('void time unknown', channelId);
      return;
    }
    if (q === 'where void') {
      const st = getState();
      ...
      if (v && v.active) {
        const link = (v && v.link)
          ...
          : (v && v.center && typeof v.center.x === 'number')
            ...
            : null;
        if (link) {
          this.broadcastBotMessage(`void at ${link}`, channelId);
          return;
        }
      }
      this.broadcastBotMessage('void location unknown', channelId);
    }
  }
