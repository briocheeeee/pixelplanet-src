...
class PcKeyProvider {
  ...
  async getKeyUsage(key) {
    ...
    // eslint-disable-next-line max-len
    this.logger.info(`PCKey: ${key}, Used Today: ${queriesToday} / ${dailyLimit} (Available: ${availableQueries}, Burst: ${availableBurst}, ${burstActive ? 'active' : 'inactive'})`);
    const keyData = [
      key,
      availableQueries,
      dailyLimit,
...
class ProxyCheck {
  ...
  reqProxyCheck(values) {
    return new Promise((resolve, reject) => {
      ...
      const payload = {};
      values.forEach((v) => { payload[v] = {}; });
      const postData = JSON.stringify(payload);

      const hasEmail = values.some((v) => typeof v === 'string' && v.includes('@'));
      const query = `vpn=1&asn=1${hasEmail ? '&email=1' : ''}&json=1&key=${key}`;
      const options = {
        hostname: 'proxycheck.io',
        path: `/v2/?${query}`,
        method: 'POST',
