...
class SocketServer {
  ...
  initialize() {
    ...
    socketEvents.on('announce', (payload) => {
      const text = `an,${JSON.stringify(payload)}`;
      this.broadcast(text);
    });
  ...
  static async onTextMessage(text, ws) {
    ...
    try {
      ...
      switch (key) {
        ...
        case 'ct': {
          // chat typing
          const channelId = Number(val[0]);
          const isTyping = !!val[1];
          if (!user || !channelId) {
            return;
          }
          const name = user.data?.name || '';
          // broadcast to all clients that have access to this channel
          const payload = `ct,${JSON.stringify([name, channelId, user.id, isTyping])}`;
          const clientArray = [];
          ws.server?.clients?.forEach((client) => {
            try {
              if (client.readyState !== 1) return;
              const cuser = client.user;
              const clang = client.lang;
              if (chatProvider.userHasChannelAccess(cuser, clang, channelId)) {
          ...
          // fallback if ws.server is not set: use this.wss from instance
          if (!clientArray.length && ws?.wss?.clients) {
          ...
          if (clientArray.length) {
            SocketServer.broadcastSelected(clientArray, payload);
          }
          break;
        }
        case 'cs': {
