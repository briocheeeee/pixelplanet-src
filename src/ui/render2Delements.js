...
import { getBrushOffsets } from '../core/brush.js';
...
export function renderPlaceholder(
  ...
  const { hover, palette, selectedColor, brushShape, brushSize, brushMatrix } = state.canvas;

  const [sx, sy] = worldToScreen(view, scale, $viewport, hover);

  viewportCtx.save();
  const offsets = getBrushOffsets(brushShape, brushSize, brushMatrix);
  for (const [dx, dy] of offsets) {
    const px = sx + dx * scale;
    const py = sy + dy * scale;
    viewportCtx.save();
    viewportCtx.translate(px + (scale / 2), py + (scale / 2));
...
export function renderPotatoPlaceholder(
  ...
  const { palette, selectedColor, hover, brushShape, brushSize, brushMatrix } = state.canvas;
  ...
  for (const [dx, dy] of offsets) {
    ...
    const py = sy + dy * scale;
    viewportCtx.fillStyle = '#000';
    viewportCtx.fillRect(px - 1, py - 1, 4, scale + 2);
    viewportCtx.fillRect(px - 1, py - 1, scale + 2, 4);
    viewportCtx.fillRect(px + scale - 2, py - 1, 4, scale + 2);
    viewportCtx.fillRect(px - 1, py + scale - 2, scale + 1, 4);
    viewportCtx.fillStyle = palette.colors[selectedColor];
    viewportCtx.fillRect(px, py, 2, scale);
    viewportCtx.fillRect(px, py, scale, 2);
    viewportCtx.fillRect(px + scale - 1, py, 2, scale);
    viewportCtx.fillRect(px, py + scale - 1, scale, 2);
  }
  viewportCtx.restore();
}
