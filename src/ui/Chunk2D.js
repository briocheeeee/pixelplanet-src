...
class Chunk2D extends Chunk {
  ...
  setColor(cell, color) {
    ...
    const c = (Number.isFinite(color)) ? (color | 0) : -1;
    const ctx = (this.image && typeof this.image.getContext === 'function')
      ? this.image.getContext('2d')
      : null;
    const colorStr = (this.palette && this.palette.colors)
      ? this.palette.colors[c]
      : undefined;
    if (!ctx || typeof ctx.fillRect !== 'function' || typeof colorStr !== 'string') return false;
