...
class PixelTransferController {
  ...
  receivePixelUpdate({
    ...
    const paletteLen = (state && state.canvas && state.canvas.palette && Array.isArray(state.canvas.palette.colors))
      ? state.canvas.palette.colors.length
      : 0;
    const maxOffset = TILE_SIZE * TILE_SIZE;
    const safePixels = Array.isArray(pixels) ? pixels : [];
    if (this.extension) {
      this.extension(i, j, safePixels);
    }
    safePixels.forEach((pxl) => {
      if (!Array.isArray(pxl) || pxl.length < 2) return;
      const [offsetRaw, colorRaw] = pxl;
      const offset = Number.isFinite(offsetRaw) ? (offsetRaw | 0) : -1;
      const color = Number.isFinite(colorRaw) ? (colorRaw | 0) : -1;
      if (offset < 0 || offset >= maxOffset) return;
      if (color < 0 || color >= paletteLen) return;
      const { clientPredictions } = this;
